...
impl BasisConfigXmlDocs {
    ...
    fn register_server_config(docs: &mut HashMap<&'static str, TypeDoc>) {
        ...
        t.fields.push(FieldDoc::new("NetworkStackId", " Transport stack id. Empty = the default ('litenetlib'); only 'litenetlib' is registered and unknown ids fall back to it. Per-stack tuning lives in config/transports/<id>.xml. string. ", None));
