...
| 2 | **ACK economy, configured on both ends** — raise the ACK-eliciting threshold and `max_ack_delay` on the clients as well as the server | the +40 % packet count above; every ACK-only packet the crowd doesn't send is a receive cycle the server doesn't run. The earlier "ack frequency changed nothing" trial predates this profile and configured the server only | **measured, then backed out** — −2.8 % packets and −1.5 % cores was not worth the configuration, and the premise was wrong in an instructive way: noq clamps `max_ack_delay` to max(RTT, 25 ms), so on loopback the default is *already* timer-bound at 25 ms and no threshold ever binds. Worth revisiting only on a real-RTT path |
