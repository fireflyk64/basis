...
namespace Basis.Network.Core
{
    ...
    public static class BasisConfigXmlDocs
    {
        ...
        private static void RegisterServerConfig()
        {
            ...
            t.Fields.Add(new FieldDoc("NetworkStackId", " Transport stack this server runs on, chosen once at startup - it does not serve two at a time, so changing it needs a restart. Empty or 'litenetlib' is the UDP protocol every shipped client speaks and is the default. 'iroh' is QUIC (encrypted, connection-migrating) through the basis_iroh_ffi native library, which must sit beside the server binary; if it is missing or built against a different ABI the server stops at boot rather than quietly serving the other protocol. On iroh a client dials an endpoint id rather than a host and port - the server prints it at boot as 'iroh clients: <id>@<host>:<port>' and reports it as the 'iroh' field of the health document - and direct peer-to-peer links are not offered, so the server relays for every pair. Per-stack tuning lives in config/transports/<id>.xml. string. "));
